...
export class PngIcoConverter {
    ...
    async convertAsync(inputs) {
        ...
        for (let i = 0; i < inputs.length; i++) {
            const currPos = FileHeaderSize + ImageHeaderSize * i, input = inputs
[i];
            const blob = this.toBlob(input.png), img = await this.loadImageAsync
(blob), w = img.naturalWidth, h = img.naturalHeight;
            if (!input.ignoreSize &&
                (w > MaxSize || h > MaxSize)) {
            ...
            // Image
            const buffer = input.png instanceof ArrayBuffer ? input.png : await
input.png.arrayBuffer();
